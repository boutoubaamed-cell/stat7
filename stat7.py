...
# ============================================
# Plotting functions (with availability checks)
# ============================================
        ...
        if factors:
            ...
            for factor_name, items in factors.items():
                ...
                if valid_items:
                    ...
                    doc.add_paragraph(
                        f'المتوسط: {factor_data.mean():.2f}' if len(factor_data) > 0 else 'المتوسط: غير متاح')
                    doc.add_paragraph(
                        f'الوسيط: {factor_data.median():.2f}' if len(factor_data) > 0 else 'الوسيط: غير متاح')
                    doc.add_paragraph(f'الانحراف المعياري: {factor_data.std():.2f}' if len(
                        factor_data) > 0 else 'الانحراف المعياري: غير متاح')
                    ...
                    if selected_var in df_analysis.columns:
                        ...
                        if test_type == "اختبار T":
                            ...
                            if len(groups) >= 2:
                                ...
                                valid_groups = []
                                for group in groups:
                                    group_data = df_analysis[df_analysis[selected_var] == group][
                                        selected_factor].dropna()
                                    if len(group_data) >= 2:
                    ...
                    if len(factors_data) >= 2:
                        ...
                        if STATS_AVAILABLE:
                            ...
                            if PLOTS_AVAILABLE:
                                ...
                                ax.set_title('مصفوفة الارتباطات')
                                st.pyplot(fig)
                                plt.close()
                        else:
                ...
                with st.spinner('جاري التجهيز...'):
                    ...
                    if export_format == "📄 CSV":
                        ...
                        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                            ...
                            if social_data:
                                pd.DataFrame(social_data).to_excel(writer, sheet_name='المتغيرات_الاجتماعية',
                                                                   index=False)
